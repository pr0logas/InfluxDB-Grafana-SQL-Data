...
influx -database l2j -execute 'DROP SERIES FROM "char_proffesions_count";'

file='./data/char_professions_count.txt'
rm -fr $file
timestamp=$(date +%s%N)
while read -a row
do
    ...
    echo "char_proffesions_count,char_name=${row[0]},char_classid=$class value=1 $timestamp" >> $file
done < <(echo "select char_name,classid from characters" | mysql l2jgs -u root)
